...
export async function createNewSession(toBeassignedUserId, data: any,) {
    try {
        ...
        for (const sessionItem of data.sessionItems) {
            ...
            if (data.sessionAgainstType == 'againstPlan') {
                sessionItemObj['activePlanId'] = new mongoose.Types.ObjectId(data.activePlanId);
            }
            if (sessionItem.trainerId) {
                sessionItemObj['trainerId'] = new mongoose.Types.ObjectId(sessionItem.trainerId);
            }
            ...
            createdUserSessions.push({ ...savedSession.toObject(), workoutItems: savedWorkoutItems })
        }
    ...
    try {
        ...
        const userSessions = await SessionModel.aggregate([
...

            {
                $lookup: {
                    from: "plans",
                    let: { planId: { $arrayElemAt: ["$activePlanDetails.plan.planId", 0] } },
                    pipeline: [
...

            {
                $lookup: {
                    from: "planitems",
                    let: { planItemId: { $arrayElemAt: ["$activePlanDetails.plan.planItemId", 0] } },
                    pipeline: [
                            ...
                            then: {
                                $mergeObjects: [
                                    { $arrayElemAt: ["$planDetails", 0] },
                                    {
                                        ...
                                        dietPlanDetails: { $arrayElemAt: ["$planDietPlanDetails", 0] },
                                    },
                                ],
                            },
                            else: "$$REMOVE",
            ...
            {
                $addFields: {
                    start: { $dateToString: { format: "%Y-%m-%d", date: "$sessionDate" } },
                    end: {
                        $dateToString: {
                            format: "%Y-%m-%d",
                            date: {
...

            {
                $addFields: {
                    color: {
                        $cond: [
                            { $eq: ["$isActive", false] }, "grey",
                            {
                                $switch: {
                                    branches: [
                                        { case: { $eq: ["$sessionStatus", "scheduled"] }, then: "orange" },
                                        { case: { $eq: ["$sessionStatus", "completed"] }, then: "green" },
                                        { case: { $eq: ["$sessionStatus", "cancelled"] }, then: "red" },
                                    ],
                                    default: "blue",
                                },
                            },
...
export async function updateSession(sessionId: string, data: Record<string, any>) {
    try {
        let user;
        if (data.trainerId) {
