...
import { assignWorkoutToSession } from "../src/sessionWorkout/sessionWorkout.service";
...
const httpTrigger: AzureFunction = async function (
  ...
  try {
    ...
    const response: { message: string; success: boolean } = await assignWorkoutToSession(sessionId,req.body);
    if (response.success) {
      context.res = {
        status: 200,
        body: response,
